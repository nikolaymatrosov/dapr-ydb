...
// These tests exercise Get/Set/Delete against a real YDB instance and are gated
// behind the `integration` build tag (like tests/integration for the binding) so
// the default `make test` / CI unit step stays green without a database. They run
// via `make ydb-up` + `go test -tags=integration ./internal/ydbstate/...`, which
// the CI integration job does against the deploy/docker-compose YDB.
package ydbstate
...
// reachable so a developer without a database still gets a clean run. The full
// behavioral matrix is also covered by the conformance suite.
